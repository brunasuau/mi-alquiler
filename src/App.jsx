...
const T = {
  es: {
    appName:"MiAlquiler",selectLang:"Elige tu idioma",loginTitle:"Bienvenido",
    email:"Correo electrónico",password:"Contraseña",login:"Entrar",logout:"Salir",
    owner:"Propietario",tenant:"Inquilino",dashboard:"Resumen",tenants:"Inquilinos",
    finances:"Finanzas",maintenance:"Mantenimiento",myHome:"Mi Piso",myCosts:"Mis Costes",
    incidents:"Incidencias",messages:"Mensajes",paid:"Pagado",pending:"Pendiente",
    markPaid:"Marcar pagado",revert:"Revertir",addCost:"Añadir coste",save:"Guardar",
    send:"Enviar",newTenant:"Nuevo inquilino",name:"Nombre completo",unit:"Piso / Habitación",
    phone:"Teléfono",rent:"Alquiler mensual (€)",createAccess:"Crear acceso",concept:"Concepto",
    amount:"Importe (€)",month:"Mes",typeMsg:"Escribe un mensaje...",
    ...
    rentStatus:"Estado del alquiler",costBreakdown:"Desglose de costes",
    paymentHistory:"Historial de pagos",registered:"Registrado el",
    dueThisMonth:"Vence este mes",inReview:"En revisión",resolved:"Resuelto",
    status:"Estado",update:"Actualizar",wrongCredentials:"Email o contraseña incorrectos",
    saving:"Guardando...",profile:"Perfil",joinedSince:"Inquilino desde",
    totalCosts:"Total costes",monthlyRent:"Alquiler fijo",incomeMonth:"Ingreso mensual",
    paidCount:"Pagos recibidos",activeTenants:"Inquilinos activos",
    pendingMaint:"Mantenimiento pendiente",recentIncidents:"Incidencias recientes",hello:"Hola",
  },
  en: {
    appName:"MyRental",selectLang:"Choose your language",loginTitle:"Welcome",
    email:"Email address",password:"Password",login:"Sign in",logout:"Sign out",
    owner:"Owner",tenant:"Tenant",dashboard:"Overview",tenants:"Tenants",
    finances:"Finances",maintenance:"Maintenance",myHome:"My Flat",myCosts:"My Costs",
    incidents:"Issues",messages:"Messages",paid:"Paid",pending:"Pending",
    markPaid:"Mark as paid",revert:"Revert",addCost:"Add cost",save:"Save",
    send:"Send",newTenant:"New tenant",name:"Full name",unit:"Flat / Room",
    phone:"Phone",rent:"Monthly rent (€)",createAccess:"Create access",concept:"Concept",
    amount:"Amount (€)",month:"Month",typeMsg:"Type a message...",
    ...
    noCosts:"No costs registered",rentStatus:"Rent status",costBreakdown:"Cost breakdown",
    paymentHistory:"Payment history",registered:"Registered on",dueThisMonth:"Due this month",
    inReview:"In review",resolved:"Resolved",status:"Status",update:"Update",
    wrongCredentials:"Wrong email or password",saving:"Saving...",profile:"Profile",
  ...
  ar: {
    appName:"إيجاري",selectLang:"اختر لغتك",loginTitle:"مرحباً",
    email:"البريد الإلكتروني",password:"كلمة المرور",login:"دخول",logout:"خروج",
    owner:"المالك",tenant:"المستأجر",dashboard:"ملخص",tenants:"المستأجرون",
    finances:"الماليات",maintenance:"الصيانة",myHome:"شقتي",myCosts:"تكاليفي",
    incidents:"البلاغات",messages:"الرسائل",paid:"مدفوع",pending:"معلق",
    markPaid:"تأكيد الدفع",revert:"تراجع",addCost:"إضافة تكلفة",save:"حفظ",
    send:"إرسال",newTenant:"مستأجر جديد",name:"الاسم الكامل",unit:"الشقة / الغرفة",
    phone:"الهاتف",rent:"الإيجار الشهري (€)",createAccess:"إنشاء حساب",concept:"البند",
    amount:"المبلغ (€)",month:"الشهر",typeMsg:"اكتب رسالة...",
    ...
    noCosts:"لا توجد تكاليف",rentStatus:"حالة الإيجار",costBreakdown:"تفاصيل التكاليف",
    paymentHistory:"سجل المدفوعات",registered:"تم التسجيل في",dueThisMonth:"مستحق هذا الشهر",
    inReview:"قيد المراجعة",resolved:"تم الحل",status:"الحالة",update:"تحديث",
    wrongCredentials:"بريد إلكتروني أو كلمة مرور خاطئة",saving:"جاري الحفظ...",
    profile:"الملف الشخصي",joinedSince:"مستأجر منذ",totalCosts:"إجمالي التكاليف",
    monthlyRent:"الإيجار الثابت",incomeMonth:"الدخل الشهري",paidCount:"المدفوعات المستلمة",
    activeTenants:"المستأجرون النشطون",pendingMaint:"صيانة معلقة",
    recentIncidents:"البلاغات الأخيرة",hello:"مرحباً",
  }
};
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (u) => {
      ...
      if (u) {
        ...
        if (snap.exists()) { setProfile(snap.data()); setLang(snap.data().lang || "es"); }
              ...
              {tenants.map(ten=>(
                <tr key={ten.id}>
                  ...
                  {months.map(m=>{
                    ...
                    return(<td key={m}><span className="badge" style={p?.paid?{background:"#E6F4ED",color:"#4A9B6F",cursor:"pointer"}:{background:"#FDECEA",color:"#D94F3D",cursor:"pointer"}} onClick={()=>onToggle(ten.id,m)}>{p?.paid?`✓ ${p.date}`:"✗ Pend."}</span></td>);
                  })}
                </tr>
              ))}
            </tbody>
      ...
      <div className="card">
        ...
        <div className="tbl-wrap">
          <table>
            ...
            <tbody>
              {tenants.flatMap(ten=>(ten.costs||[]).map(c=>(<tr key={c.id}><td>{ten.name}</td><td>{c.icon} {c.name}</td><td>{c.month}</td><td>{c.amount}€</td></tr>)))}
            </tbody>
          </table>
        </div>
...
function TenantMessages({ t, tenantId }) {
  ...
  return (
    <div>
      <div className="page-hd"><h2>{t.messages}</h2></div>
      <ChatWindow roomId={[ownerId,tenantId].sort().join("_")} senderId={tenantId} t={t} />
    </div>
  );
}
...
function ChatWindow({ roomId, senderId, t }) {
  ...
  const sendMsg=async()=>{
    if(!text.trim()) return;
    await addDoc(collection(db,"chats",roomId,"messages"),{text:text.trim(),senderId,createdAt:serverTimestamp()});
    setText("");
  };
  return (
      ...
      <div className="card">
        ...
        {months.length===0?<p style={{color:"var(--warm)",fontSize:14}}>{t.pending}</p>:
          months.map(m=>{
            ...
            return(<div key={m} className="cr"><div className="cn">{m}</div><span className="badge" style={pm.paid?{background:"#E6F4ED",color:"#4A9B6F"}:{background:"#FDECEA",color:"#D94F3D"}}>{pm.paid?`✓ ${pm.date}`:"✗ Pendiente"}</span></div>);
          })}
      </div>
    </div>
  );
