...
function ExtractoTab({tenants, onToggle, monthsOfYear}) {
  ...
  const parseExtracto = (text) => {
    ...
    for(const line of lines){
      if(/fecha|date|concepto|importe|saldo|descrip/i.test(line) && movs.length===0) continue;
      ...
      if(parts.length >= 3){
        for(let i=parts.length-1; i>=0; i--){
          const raw = parts[i].replace(/\s/g,"").replace(/\.(?=\d{3})/g,"").replace(",",".");
          const n = parseFloat(raw);
          if(!isNaN(n) && Math.abs(n)>0.01 && Math.abs(n)<999999){
            importe = n;
            descripcion = parts.filter((_,j)=>j!==i).join(" ").replace(dateMatch[0],"").replace(/\s{2,}/g," ").trim();
            break;
          }
        }
      }

      // Fallback: scan for number pattern in free text
      if(importe===null){
        const re = /([+-]?\d{1,3}(?:[.,]\d{3})*[.,]\d{2})/g;
        let m; const found=[];
        while((m=re.exec(line))!==null) found.push(parseFloat(m[1].replace(/\.(?=\d{3})/g,"").replace(",",".")));
        if(found.length) { importe=found[found.length-1]; descripcion=line.replace(dateMatch[0],"").slice(0,80).trim(); }
      }
      ...
      if(/alquiler|lloguer|arrendamiento|renta/.test(ll)||( tenantMatch&&tipo==="ingreso")) concepto="alquiler";
      else if(/luz|electricidad|endesa|iberdrola|gas nat|agua|suministro/.test(ll)) concepto="suministro";
      else if(/reparaci|manten|obra|fontanero|electricista/.test(ll)) concepto="mantenimiento";

      movs.push({fecha, descripcion:descripcion||line.slice(0,70), importe:abs, tipo, tenantMatch, concepto});
    }
    return movs;
  };
  ...
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState(null); // {movimientos:[{fecha,descripcion,importe,tipo,match}]}
  const [applied, setApplied] = useState({});
  const [selMonth, setSelMonth] = useState(monthsOfYear[new Date().getMonth()]||monthsOfYear[0]);
  const monthNames=["Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre"];
  ...
  const handleFile = async(e) => {
    ...
    // Read file as text (CSV/TXT) or as base64 (PDF)
    if(f.type === "application/pdf"){
      const reader = new FileReader();
      reader.onload = ev => setFileText({type:"pdf", data:ev.target.result.split(",")[1]});
      reader.readAsDataURL(f);
    } else {
      const reader = new FileReader();
      reader.onload = ev => setFileText({type:"text", data:ev.target.result});
      reader.readAsText(f, "latin1");
    }
  };

  const tenantSummary = tenants.map(t=>({name:t.name, rent:t.rent, unit:t.unit, building:t.building}));

  const analyse = async() => {
    if(!fileText) return;
    setLoading(true);
    try{
      const msgs = fileText.type==="pdf"
        ? [{role:"user", content:[
            {type:"document", source:{type:"base64", media_type:"application/pdf", data:fileText.data}},
            {type:"text", text:`Estos son los inquilinos: ${JSON.stringify(tenantSummary)}. Analiza el extracto bancario y devuelve SOLO JSON sin markdown con este formato exacto: {"movimientos":[{"fecha":"DD/MM/YYYY","descripcion":"texto original","importe":123.45,"tipo":"ingreso|gasto","tenantMatch":"nombre del inquilino si coincide o null","concepto":"alquiler|suministro|mantenimiento|otro"}]}. Clasifica como ingreso si el importe es positivo (entradas), gasto si es negativo (salidas). Intenta cruzar cada ingreso con un inquilino por nombre o importe de renta.`}
          ]}]
        : [{role:"user", content:`Estos son los inquilinos: ${JSON.stringify(tenantSummary)}.\n\nEste es el extracto bancario:\n${fileText.data}\n\nDevuelve SOLO JSON sin markdown con este formato exacto: {"movimientos":[{"fecha":"DD/MM/YYYY","descripcion":"texto original","importe":123.45,"tipo":"ingreso|gasto","tenantMatch":"nombre del inquilino si coincide o null","concepto":"alquiler|suministro|mantenimiento|otro"}]}. Clasifica como ingreso si el importe es positivo, gasto si es negativo. Cruza cada ingreso con un inquilino por nombre o importe de renta.`}];
  ...
  const totalIngresos = (result?.movimientos||[]).filter(m=>m.tipo==="ingreso").reduce((s,m)=>s+Math.abs(m.importe),0);
  const totalGastos = (result?.movimientos||[]).filter(m=>m.tipo==="gasto").reduce((s,m)=>s+Math.abs(m.importe),0);
  const ingresosMarcados = Object.values(applied).filter(a=>a==="ingreso").length;
  const gastosMarcados = Object.values(applied).filter(a=>a==="gasto").length;

  const applyAll = async() => {
    if(!result) return;
    const movs = result.movimientos;
    for(const mov of movs){
      if(mov.tipo==="ingreso" && mov.tenantMatch){
        const ten = tenants.find(t=>t.name===mov.tenantMatch);
        if(ten && !((ten.payments||{})[selMonth]?.paid)){
          await onToggle(ten.id, selMonth);
        }
      }
    }
    // Mark all as applied
    const newApplied = {};
    movs.forEach((_,i)=>newApplied[i]="done");
    setApplied(newApplied);
    alert(`✅ Marcados ${movs.filter(m=>m.tipo==="ingreso"&&m.tenantMatch).length} pagos como cobrados en ${selMonth}`);
  };

  const conceptColors = {alquiler:"#4A9B6F",suministro:"#4F46E5",mantenimiento:"#D4A853",otro:"#8C7B6E"};
  const conceptLabels = {alquiler:"🏠 Alquiler",suministro:"💡 Suministro",mantenimiento:"🔧 Mantenimiento",otro:"📋 Otro"};
  ...
  return(
    <div>
      ...
      <div className="card" style={{marginBottom:16}}>
        <div className="card-title">🏦 Extracto bancario</div>
        <p style={{fontSize:13,color:"var(--warm)",marginBottom:14}}>
          Sube el extracto del banco (PDF o CSV/TXT) y la app identificará automáticamente los pagos de los inquilinos y los gastos.
        </p>
        <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap",marginBottom:12}}>
          <label style={{
            display:"flex",alignItems:"center",gap:8,padding:"10px 18px",
            background:"var(--terra)",color:"white",borderRadius:10,cursor:"pointer",fontSize:13,fontWeight:600
          }}>
            📎 {file?file.name:"Seleccionar archivo"}
            <input type="file" accept=".pdf,.csv,.txt,.xls,.xlsx" onChange={handleFile} style={{display:"none"}}/>
          </label>
          ...
          <button className="btn btn-p" onClick={analyse} disabled={!fileText||loading}>
            {loading?"⏳ Analizando...":"🔍 Analizar extracto"}
          </button>
        </div>
        {file&&!result&&!loading&&<p style={{fontSize:12,color:"#4A9B6F"}}>✅ Archivo cargado. Pulsa "Analizar extracto".</p>}
      </div>
      ...
      {result&&(
        <>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:12,marginBottom:16}}>
            <div className="card" style={{padding:14,textAlign:"center",border:"2px solid #4A9B6F"}}>
              <div style={{fontSize:22,fontWeight:700,color:"#4A9B6F"}}>{totalIngresos.toFixed(2)}€</div>
              <div style={{fontSize:11,color:"var(--warm)",textTransform:"uppercase"}}>💰 Ingresos</div>
            </div>
            <div className="card" style={{padding:14,textAlign:"center",border:"2px solid #D94F3D"}}>
              <div style={{fontSize:22,fontWeight:700,color:"#D94F3D"}}>{totalGastos.toFixed(2)}€</div>
              <div style={{fontSize:11,color:"var(--warm)",textTransform:"uppercase"}}>📤 Gastos</div>
            </div>
            <div className="card" style={{padding:14,textAlign:"center",border:"2px solid var(--terra)"}}>
              <div style={{fontSize:22,fontWeight:700,color:"var(--terra)"}}>{(totalIngresos-totalGastos).toFixed(2)}€</div>
              <div style={{fontSize:11,color:"var(--warm)",textTransform:"uppercase"}}>📊 Balance</div>
            </div>
            <div className="card" style={{padding:14,textAlign:"center",border:"2px solid #4F46E5"}}>
              <div style={{fontSize:22,fontWeight:700,color:"#4F46E5"}}>{result.movimientos.filter(m=>m.tenantMatch).length}/{result.movimientos.filter(m=>m.tipo==="ingreso").length}</div>
              <div style={{fontSize:11,color:"var(--warm)",textTransform:"uppercase"}}>🏠 Inquilinos ID</div>
            </div>
          </div>

          {/* Apply all button */}
          {result.movimientos.some(m=>m.tipo==="ingreso"&&m.tenantMatch)&&(
            <div style={{marginBottom:16,display:"flex",gap:10,alignItems:"center",background:"#F0FAF4",border:"1.5px solid #4A9B6F",borderRadius:12,padding:"12px 16px"}}>
              <div style={{flex:1,fontSize:13}}>
                <strong style={{color:"#4A9B6F"}}>✅ {result.movimientos.filter(m=>m.tipo==="ingreso"&&m.tenantMatch).length} pagos identificados</strong>
                <span style={{color:"var(--warm)",marginLeft:8}}>de {result.movimientos.filter(m=>m.tipo==="ingreso").length} ingresos totales · mes: <strong>{selMonth}</strong></span>
              </div>
              <button className="btn btn-p" onClick={applyAll}>✅ Marcar todos como cobrados</button>
            </div>
          )}

          {/* Movements list */}
          <div className="card">
            <div className="card-title">📋 Movimientos ({result.movimientos.length})</div>
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {result.movimientos.map((mov,i)=>{
                const isIngreso = mov.tipo==="ingreso";
                const tenant = mov.tenantMatch ? tenants.find(t=>t.name===mov.tenantMatch) : null;
                const alreadyPaid = tenant && ((tenant.payments||{})[selMonth]?.paid);
                const wasApplied = applied[i]==="done";
                return(
                  <div key={i} style={{
                    display:"flex",alignItems:"center",gap:12,padding:"12px 14px",
                    borderRadius:10,border:`1.5px solid ${isIngreso?"#C8E6C9":"#FFCDD2"}`,
                    background:isIngreso?"#F9FFF9":"#FFF8F8",flexWrap:"wrap"
                  }}>
                    <div style={{fontSize:20}}>{isIngreso?"💚":"🔴"}</div>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontWeight:600,fontSize:13,marginBottom:2}}>{mov.descripcion}</div>
                      <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
                        <span style={{fontSize:11,color:"var(--warm)"}}>{mov.fecha}</span>
                        <span style={{fontSize:11,background:conceptColors[mov.concepto]||"#8C7B6E",color:"white",padding:"2px 8px",borderRadius:20}}>
                          {conceptLabels[mov.concepto]||mov.concepto}
                        </span>
                        {mov.tenantMatch&&(
                          <span style={{fontSize:11,background:"#E8F5E9",color:"#2E7D32",padding:"2px 8px",borderRadius:20,fontWeight:600}}>
                            🏠 {mov.tenantMatch}
                          </span>
                        )}
                        {alreadyPaid&&<span style={{fontSize:11,color:"#4A9B6F"}}>✅ Ya marcado</span>}
                        {wasApplied&&<span style={{fontSize:11,color:"#4A9B6F"}}>✅ Aplicado</span>}
                      </div>
                    </div>
                    <div style={{fontWeight:700,fontSize:16,color:isIngreso?"#4A9B6F":"#D94F3D",flexShrink:0}}>
                      {isIngreso?"+":"-"}{Math.abs(mov.importe).toFixed(2)}€
                    </div>
                    {isIngreso&&tenant&&!alreadyPaid&&!wasApplied&&(
                      <button className="btn btn-s btn-sm" onClick={async()=>{
                        await onToggle(tenant.id, selMonth);
                        setApplied(a=>({...a,[i]:"done"}));
